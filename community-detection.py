...
def my_girvan_newman_for_communities(G, graph_layout, node_positions):
    global community_tuples, LC, LC1, LC2, num_edges_removed
    start_time = time.time()

    connected_components = nx.connected_components(G)
    my = nx.number_connected_components(G)
    ...
    print(bcolors.ENDC  + "\t===================================================")
    print(bcolors.ENDC  + "\tComputation of Girvan-Newman clustering for a graph with",G.number_of_nodes(),"nodes and",G.number_of_edges(),"links. "
...
def divisive_community_detection(G,number_of_divisions,graph_layout,node_positions):
    ...
    for division in range(number_of_divisions): # the user chooses the number of partitions
        triplet.clear()
        triplet.append(LC)
        # call my method and get the rest of the global variables to put in the triplets
        my_girvan_newman_for_communities(G,graph_layout, node_positions)
        triplet.append(LC1)
        triplet.append(LC2)
    ...
    print('     The number of edges removed is: ', number_of_divisions)
    print('     The number of communities from my own algorithm is: ', len(community_tuples))
